...
// RefusalFailoverConfig re-runs a refused LLM call on a different model and
// holds that model as the turn-start model for a window afterwards. Intended
// for a primary whose safety classifier false-positives on legitimate traffic.
// A nil config or empty Model disables the feature entirely.
type RefusalFailoverConfig struct {
	Model       string `json:"model,omitempty"`        // model_name from model_list to fail over to
	HoldMinutes int    `json:"hold_minutes,omitempty"` // turn-start hold window; <= 0 defaults to 360
}

// HoldDuration returns the window during which turns start on the failover model.
func (c *RefusalFailoverConfig) HoldDuration() time.Duration {
	if c == nil || c.HoldMinutes <= 0 {
...
type AgentDefaults struct {
	...
	RefusalFailover           *RefusalFailoverConfig `json:"refusal_failover,omitempty"`
}
